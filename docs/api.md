...
| Method | Path                              |                                                                                                                                                                                                    |
| ------ | --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET    | `/api/v1/overflights`             | filters: `location_id`, `from`/`to` (local dates), `registration`, `type_code`, `operator` (ICAO), `status`, `include_position=true` (closest-approach coordinates); newest closest-approach first |
...
| GET       | `/admin/v1/seen-aircraft`                       | airframes seen overhead, most-seen first, plus passes by type and by operator (top 50 each), each airframe's recent routes, and your photo collection (best photo per operator + type seen); `owner_id`, `days` (30), `include_near_misses`, `type_code`, `operator`, `manufacturer` (prefixes, e.g. `airbus,boeing`), `exclude_helicopters`, `limit` (200, max 500) |
| GET       | `/admin/v1/aircraft-photos/{icao24}`            | Planespotters.net photo by Mode S address, then `registration`; cached (12 h, misses 1 h). `not_ready` when unconfigured or Planespotters is unreachable                                                                                                                                                                                                             |
| GET       | `/admin/v1/aircraft-photos/{icao24}/candidates` | every photo on offer: Planespotters.net, adsbdb (airport-data.com) and Wikimedia Commons (by `registration`); `failed` lists sources that could not be reached                                                                                                                                                                                                       |
| POST      | `/admin/v1/aircraft-photos/{icao24}/picks`      | save one offered photo (`image_url` must match a candidate) as a source image, links only; the latest pick shows on the Aircraft page. Fills your operator + type collection slot when it is empty; otherwise `collection` returns the current best to compare                                                                                                       |
| PUT       | `/admin/v1/photo-collection`                    | make one of your saved photos (`source_image_id`) the best for its operator + type slot                                                                                                                                                                                                                                                                              |
| DELETE    | `/admin/v1/aircraft-photos/picks/{id}`          | remove a saved photo                                                                                                                                                                                                                                                                                                                                                 |
| GET       | `/admin/v1/source-images`                       | reference photos with licence/attribution and image links                                                                                                                                                                                                                                                                                                            |
| POST      | `/admin/v1/source-images/{id}/cutout`           | queue (or redo) a background-removed copy for the worker; refused (`validation_failed`) when the licence does not allow edited copies. `/source-images` items carry `cutout` (status, signed PNG link) and `cutout_refusal`                                                                                                                                          |
