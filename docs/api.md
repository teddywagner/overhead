...
| GET       | `/admin/v1/seen-aircraft`                       | airframes seen overhead, most-seen first, plus passes by type and by operator (top 50 each); `owner_id`, `days` (30), `include_near_misses`, `type_code`, `operator`, `manufacturer` (prefixes, e.g. `airbus,boeing`), `exclude_helicopters`, `limit` (200, max 500) |
...
| POST      | `/admin/v1/aircraft-photos/{icao24}/picks`      | save one offered photo (`image_url` must match a candidate) as a source image, links only; the latest pick shows on the Aircraft page                                                                                                                                |
