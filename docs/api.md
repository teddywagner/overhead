...
| GET       | `/admin/v1/seen-aircraft`                       | airframes seen overhead, most-seen first, plus passes by type and by operator (top 50 each); `owner_id`, `days` (30), `include_near_misses`, `type_code`, `operator`, `manufacturer` (prefixes, e.g. `airbus,boeing`), `limit` (200, max 500) |
