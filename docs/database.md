...
| `photo_collection`        | An admin's best saved photo (a `photo_pick` source image) per operator + aircraft type slot; `operator_icao` null is the unknown-operator slot. Removing the photo empties the slot.                                                                                                                                                                      |
| `image_cutouts`           | Queue and record of background-removed copies of source images: `pending` → `processing` → `done` (PNG at `<owner>/cutout/<source_image_id>.png` in `source-images`) or `failed` (retried with back-off, 3 attempts). One per source image.                                                                                                               |
...
| photo_collection               | select                                                                        | own; written only by the API                                                     |
| image_cutouts                  | select                                                                        | own; written only by the API and worker                                          |
